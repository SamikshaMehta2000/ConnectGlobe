 ...
 <section class="aw-accordion">
   ...
   <div id="nav" class="read-article">
    				<ul>
						...
						<li><a href="Advice.jsp" style="color:white;">YOUR ADVICE</a></li>
  ...
  <div class="four" style="  background-color:#9ea036;"> 
	...
	<div id="basic">
				...
				All the reports are listed here , feel free to give your advice on these.</h1>
	</div>
  ...
  <!--      CONNECTIVITY CODE	 -->
				 ...
				 try{
					...
					out.print("<font class='item'>"+rs.getString("postedon")+"</font>");
					  ...
					  ps=con.prepareStatement("select max(id) from forumadvc where rid="+rs.getInt("id"));
					  ResultSet rs1=ps.executeQuery();
					  rs1.next();
					  int max=rs1.getInt(1);
					  
					  //find total comments related to a post
					  ps=con.prepareStatement("select count(id) from forumadvc where rid="+rs.getInt("id"));
					  ...
					  ps=con.prepareStatement("select * from forumadvc where rid="+rs.getInt("id")+" limit 3");
...
<script type="text/javascript">
  ...
  //function to view all comments
  			function viewAll(name){
				  ...
				  $.get("Allcmts.jsp",{n:name},function(dataa){
					  $(d).html(dataa);
  ...
  //function to post a comment
  			...
  			function post(n){
				   ...
				   var id="#"+n+"abc";
					   $(id).html("<form action='Postcmt.jsp' id='loginform'><input type='text'  name='id' style='display:none' value='"+n+"'><textarea style='background-color:#90a4ae;' rows=5 cols=110 name='cmt'/><br><input type='submit' class='btn' style='color:#c9d6e2' value='comment'/></form> ");
	  		 }
